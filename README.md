...
The build tools require [Node.js](http://nodejs.org/), [npm](http://npmjs.org/) and [Bower](http://bower.io/). Install dependencies using gulp `npm install; bower install`, then use `gulp make` to build. You can also test files by extensions (`gulp css`, `gulp js` & `gulp php`) or make use of `gulp lint`.
